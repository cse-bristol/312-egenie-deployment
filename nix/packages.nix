{pkgs, pythonPackages}:
let
  commonBuildInputs = with pkgs; [ mariadb zlib openssl pkgconfig libjpeg openjpeg libtiff freetype lcms2 libwebp tcl ncurses pythonPackages.setuptools_scm pythonPackages.nose ];
  commonDoCheck = false;
  python = {mkDerivation = pythonPackages.buildPythonPackage;};
  self =
  {
    ...
    #   doCheck = commonDoCheck;
    #   LC_ALL="en_US.utf-8";
    #   buildInputs = commonBuildInputs ++ [pkgs.glibcLocales];
    #   propagatedBuildInputs = [
    #   self."Unidecode"
    #   self."six"
    # ];
