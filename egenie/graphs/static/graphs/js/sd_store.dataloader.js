...
sd_store.dataloader = (function () {
    ...
    load_everything = function (p, on_data_loaded) {
        ...
        load_by_deployment = function (deployment) {
            return load_readings(deployment.sensors);
        };
        ...
        on_everything_loaded = function () {
            ...
            for (i = 0; i<checked_arguments.length; i += 1) {
                ...
                if (p.hasOwnProperty('annotations') === true) {
        ...
        } else if(p.hasOwnProperty('deployment')) {
            console.log("Load deployment");
            chained_load_readings = $.ajax({
                url: p.deployment_url + p.deployment + '/devices',
                data: p,
            }).then(load_by_deployment);
